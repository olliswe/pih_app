...
class ExpenseQueryset(models.QuerySet):
    def need_organizing(self):
        return self.filter(
            organized_by_us=True, is_organized=False, request_form__status="approved"
        )

    def outstanding(self):
        return self.filter(
            Q(expenses_covered_by="Covered by us but reimbursed")
    ...
    def need_reimbursement(self):
        return self.filter(
            expenses_covered_by="Covered by us but reimbursed",
            expense_paid=True,
...
class Request(models.Model):
    ...
    host = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name="Requestor/Host",
    )
    departure_place = models.CharField(
        max_length=225, verbose_name="Place of Departure", choices=DEPARTURE_CHOICES
    )
    other_place = models.CharField(
        max_length=225,
        verbose_name="Other Place of Departure (please specify)",
        null=True,
        blank=True,
    )
    ...
    submission_date = models.DateTimeField(
        auto_now_add=True, verbose_name="Submitted on"
    )
    num_visitors = models.IntegerField(verbose_name="Number of Visitors")
    review_comment = models.TextField(
        max_length=1000, verbose_name="Review comment", null=True, blank=True
    )
    ...
    def is_archived(self):
        if self.status == "review":
            ...
            self.organization_status() == "completed"
            and self.reimbursement_status() == "completed"
...
class Expense(models.Model):
    ...
    organized_by_us = models.BooleanField(
        default=False, verbose_name="Organized by us?"
    )
    name_of_organizer = models.ForeignKey(
        User,
        on_delete="SET_DEFAULT",
        default=None,
        null=True,
        blank=True,
        verbose_name="Name of Organizer",
    )
    expenses_covered_by = models.CharField(
        max_length=225, choices=EXPENSES_CHOICES, verbose_name="Expenses covered by"
    )
    expenses_amount = models.FloatField(
        verbose_name="Estimated expenses amount in USD", null=True, blank="True"
    )
    notes = models.TextField(
        max_length=225, verbose_name="Additional Notes", null=True, blank=True
    )
    request_form = models.ForeignKey(
        Request, verbose_name="Request", on_delete="CASCADE"
    )

    is_organized = models.BooleanField(default=False, verbose_name="Has been organized")
    marked_as_organized_by = models.ForeignKey(
        User,
        related_name="marked_as_organized_by",
        verbose_name="Marked as organized by",
        on_delete="SET_DEFAULT",
        default=None,
        null=True,
        blank=True,
    )
    marked_as_organized_on = models.DateField(
        verbose_name="Marked as organized on", null=True, blank=True
    )

    expense_reimbursed = models.BooleanField(
        default=False, verbose_name="Expense has been reimbursed"
    )
    marked_as_reimbursed_by = models.ForeignKey(
        User,
        related_name="marked_as_reimbursed_by",
        verbose_name="Marked as reimbursed by",
        on_delete="SET_DEFAULT",
        default=None,
        null=True,
        blank=True,
    )
    marked_as_reimbursed_on = models.DateField(
        verbose_name="Marked as reimbursed on", null=True, blank=True
    )
    amount_reimbursed = models.FloatField(
        verbose_name="Amount reimbursed", default=None, null=True, blank=True
    )
    ...
    def needs_to_be_organized(self):
        if (
            self.organized_by_us
            and not self.is_organized
            and self.request_form.status == "approved"
        ):
            return True
        else:
            return False
